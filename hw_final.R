...
print(paste("The degree chosen by the BIC criterion is given by:", best_deg_BIC))
...
# since it seems to capture the general shape of the data well but the RSE is worse.

# Question 4

x_grid <- seq(min(data_3$x), 10, length.out = 200)



grid_conf <- predict(chosen_polynomial, newdata = data.frame(x = x_grid), interval = "confidence")

grid_pred <- predict(chosen_polynomial, newdata = data.frame(x = x_grid), interval = "prediction")


x_table_data <- data.frame(x = 1:10)

tab_conf <- predict(chosen_polynomial, newdata = x_table_data, interval = "confidence")


tab_pred <- predict(chosen_polynomial, newdata = x_table_data, interval = "prediction")
...
cat("\n--- Résultats du polynôme de degré 5 ---\n")
print(round(final_table, 3))

plot(data_3$x, data_3$y, pch = 21, cex = 1.5, lwd = 2, col = "darkblue", bg = "skyblue",
     xlim = c(min(data_3$x), 10), 
     ylim = range(grid_pred), 
     xlab = "x", ylab = "y",
     main = "Intervalles de Confiance et de Prédiction (Polynôme)")


polygon(c(x_grid, rev(x_grid)), c(grid_pred[, "lwr"], rev(grid_pred[,"upr"])), 
        col = rgb(0.5, 0, 0.5, 0.1), border = NA)

polygon(c(x_grid, rev(x_grid)), c(grid_conf[,"lwr"], rev(grid_conf[,"upr"])), 
        col = rgb(0, 0.5, 0, 0.2), border = NA)


lines(x_grid, grid_conf[, "fit"], lwd = 3, col ="firebrick") 

lines(x_grid, grid_conf[,"lwr"], col = "darkgreen", lty=2) 

lines(x_grid, grid_conf[,"upr"], col = "darkgreen", lty= 2)
lines(x_grid, grid_pred[,"lwr"], col = "purple", lty =3)    

lines(x_grid, grid_pred[,"upr"], col = "purple", lty= 3)


points(x_table_data$x, tab_conf[, "fit"], pch = 4, col = "black", lwd = 2)

legend("topleft", 
       legend = c("Données", "Modèle (Fit)", "IC : E(Ynew)", "IP : Ynew", "Points prédits (1:10)"),
       fill = c(NA, NA, rgb(0, 0.5, 0, 0.2), rgb(0.5, 0, 0.5, 0.1), NA),
       border = NA,
       col = c("darkblue", "firebrick", "darkgreen", "purple", "black"),
       pch = c(21, NA, NA, NA, 4),
       lty = c(NA, 1, 2, 3, NA),
       bty = "n", cex = 0.8)
...
#Function to visualize the density:
...
# Due to the large amount of data (4301 data points), the central limit theorem allows us to use a t-test.
